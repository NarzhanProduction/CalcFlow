...
func pingHandler(w http.ResponseWriter, r *http.Request) {
	...
	if agentID == "" {
		http.Error(w, "The Agent ID is required", http.StatusBadRequest)
		return
	}

	// Получаем имя пользователя, чтобы проверить в checkFreeExpressions
	receivedUserName := r.Header.Get("Authorization")

	user := strings.TrimPrefix(receivedUserName, "Bearer ")
	if user == "" {
	...
	log.Printf("Received ping from agent %s of %s", agentID, user)
...
func computeExpression(expression, user string, id, op1, op2, op3, op4, op5 int) (int, error) {
	...
	requestData := map[string]string{"expression": expression, "op1": fmt.Sprint(op1), "op2": fmt.Sprint(op2), "op3": fmt.Sprint(op3), "op4": fmt.Sprint(op4), "op5": fmt.Sprint(op5)}
	...
	resp, err := http.Post(fmt.Sprintf("http://localhost:%d", port), "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return 0, fmt.Errorf("ошибка при создании запроса: %v", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("ошибка вычисления выражения: код статуса %d", resp.StatusCode)
	}
...
func main() {
	...
	http.HandleFunc("/createAgents", agentsCreater)
	http.HandleFunc("/ping", func(w http.ResponseWriter, r *http.Request) {
		// Твой код обработки здесь
		pingHandler(w, r)
	})
	// http.HandleFunc("/agents", agentsHandler)
